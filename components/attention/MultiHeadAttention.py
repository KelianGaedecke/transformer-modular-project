...
class MultiHeadAttention(AttentionStrategy):
    ...
    def forward(
        ...
        context: Optional[torch.Tensor]        = None,  # reserved for cross-attention
        ...
        # 4. Additive attention bias from PE (e.g. distance bias, ALiBi)
        if pe is not None and coords is not None:
            bias = pe.get_bias(coords)
            if bias is not None:
                attn_scores = attn_scores + bias

        # Full (non-causal) attention — every point attends to every other point.
